...
mycode=$(cat /home/pi/$MYDIR/config.json | jq -r '.chainlink_token')
...
while : ;do
  usdreading=$(eval "/home/pi/$MYDIR/chainlink_pricefinder.py $mycode" )
  #usdraw=$(curl -s -X GET "https://api.coingecko.com/api/v3/simple/price?ids=$mycode&vs_currencies=usd" -H "accept: application/json")
  #if echo "$usdraw" | grep -q '{"usd":'  ;then usdreading=$(echo "$usdraw" |jq -r '."'$mycode'".usd') #data scrape was successful
    usdreading=$(echo "$usdreading * $mymod" |bc)
