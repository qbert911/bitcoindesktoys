...
def show_other_exchanges():
    ...
    print(Back.CYAN+Fore.BLUE+Style.DIM+"F"+str(format(iusdc_interest, '05.2f'))+"% C"+str(format(crusdc_interest, '04.1f'))+"%"+Style.RESET_ALL, end=' - ')  #+"C"+str(crusdc_interest)+"% "+Fore.MAGENTA+Style.BRIGHT+"Ç"+str(format(crcrv_interest, '.2f'))+"%"
...
def header_display():
    ...
    for i in range(0, len(carray["name"])):
        ...
        print(carray["longname"][i].ljust(len(max(carray["longname"], key=len))), carray["name"][i], str(carray["invested"][i]).rjust(cw[0]),
              str(format(carray["balanceof"][i], '.2f')).rjust(cw[1]), str(format(carray["totalsupply"][i], '.0f')).rjust(cw[2]),
...
def main():
    ...
    while True:                                                                     #Initiate main program loop
        ...
        for i in range(0, len(carray["name"])):
            ...
            if myarray[-1][carray["name"][i]+"pool"] > mydict[carray["name"][i]+"pool"]: #debug lines ... should not happen
